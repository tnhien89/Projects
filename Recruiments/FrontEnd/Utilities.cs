...
namespace FrontEnd
{
    public class Utilities
    {
        ...
        public static int GetRequestParam(string key)
        {
            try
            {
                ...
                return int.Parse(HttpContext.Current.Request.Params[key]);
            }
            catch (Exception ex)
            {
        ...
        public static string ReplaceImageUri(int id, string html, string dir)
        {
            try
            {
                LogHelpers.LogHandler.Info(string.Format("id: {0}, html: {1}, dir: {2}",
                    id, html, dir));
        ...
        public static string CreateImagesUrl(int id, string html, string rootImageUrl)
        {
            try
            {
                LogHelpers.LogHandler.Info(string.Format("rootImageUrl: {0}",
                    rootImageUrl));
                ...
                foreach (HtmlNode node in htmlDoc.DocumentNode.SelectNodes("//img[@src]"))
                {
                    ...
                    string fullPath = Path.Combine(rootImageUrl, id.ToString(), src);
        ...
        public static string GetSiteLogoUrl(string logoName)
        {
            LogHelpers.LogHandler.Info("Start");
            try
            {
                return Path.Combine(ConfigurationManager.AppSettings["UploadImagesUrl"], "Logo", logoName);
            }
            catch (Exception ex)
            {
                LogHelpers.LogHandler.Error(ex.ToString());
