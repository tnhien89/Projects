...
namespace AdminSite.UserControls
{
    public partial class ucSubOtherItems : System.Web.UI.UserControl
    {
        ...
        protected void grvSubOtherItems_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (_catId <= 0)
            {
                _catId = Utilities.GetRequestParameter("Id");
            }
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(imgLink.ImageUrl))
                {
                    imgLink.ImageUrl = "~/" + Path.Combine(Utilities.GetDirectory("ImagesDir"),
                        imgLink.ImageUrl.Split('|')[0]);
                }
        ...
        {
            Response.Redirect("OtherDetail.aspx?CatId=" + _catId.ToString(), false);
        }
    }
}