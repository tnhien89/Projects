...
{
    public partial class ucGalleryItems : System.Web.UI.UserControl
    {
        ...
        protected void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                Response.Redirect(string.Format("~/{0}?MenuId={1}", Utilities.GetNavigationUrl("GalleryDetail"),
                    _menuId), false);
            }
            catch (Exception ex)
            {
        ...
        protected void grvProjectItems_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            try
            {
                ...
                imgLink.ImageUrl = "~/" + Path.Combine(Utilities.GetDirectory("ImagesDir"), imgLink.ImageUrl.Split('|')[0]);
            }
            catch (Exception ex)
            {
